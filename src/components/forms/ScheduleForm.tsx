...
export function ScheduleForm ({
  ...
  const groupedAvailabilityFields = Object.groupBy(
    availabilityFields.map((field, index) => ({ ...field, index })), // add index to each field
    field => field.dayOfWeek, // the key to group by
  )

  async function onSubmit(values: z.infer<typeof scheduleFormSchema>) {
    const res = await saveSchedule(values)

    if (res?.error) {
      form.setError("root", {
        message: `There was an error saving your schedule: ${res.error}`
      })
    } else {
